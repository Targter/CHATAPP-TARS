...
export function ChatSettingsDialog({
  ...
  }, [currentName, currentTheme, currentTimer, isOpen]);
  ...
  const handleSave = async () => {
    setIsLoading(true);
    try {
      // FIX: Single, clean mutation call
      await updateSettings({
        conversationId,
        themeColor: color,
        name: name, // Handled correctly by backend depending on isGroup/targetUserId
        targetUserId: partnerId,
        disappearingTimer: timer === 0 ? undefined : timer, // Clear if 0
      });
      onClose();
    } catch (error) {
      console.error("Failed to update settings:", error);
        ...
        <div className="space-y-6 py-4">
          ...
          <div className="space-y-3">
            ...
            <Select
              ...
              onValueChange={(val) => setTimer(Number(val))}
            >
              <SelectTrigger className="w-full bg-background/50 backdrop-blur-md border-border">
                <SelectValue placeholder="Select timer" />
              </SelectTrigger>
